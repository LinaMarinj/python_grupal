...
datos_filtrados = data[
    (data["EDAD FALLECIDO"] >= rango_edad[0])
    & (data["EDAD FALLECIDO"] <= rango_edad[1])
]
...
fig2 = px.histogram(
    datos_filtrados, x="EDAD FALLECIDO", nbins=20, title="Distribución de Edades"
)
st.plotly_chart(fig2, use_container_width=True)

# 3. Gráfico de causas
fig3 = px.bar(
    datos_filtrados["CAUSA DIRECTA"].value_counts().head(10),
    orientation="h",
...
fig5 = px.line(
    datos_filtrados["FECHA DEFUNCION"].value_counts(),
